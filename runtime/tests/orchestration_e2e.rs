use contract::*;
use runtime::{
    CapabilityProvider, LocalModel, LocalProvider, OrchestrationRequest, Orchestrator,
    ResolvedConfig, assembler::DefaultPromptAssembler,
    capability_parser::DefaultCapabilityResponseParser, context::NullContextSource,
    parser::DefaultOutputParser,
};
use std::path::PathBuf;

fn test_model_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../providers/llama-cpp/fixtures")
        .join("qwen2.5-0.5b-instruct-q4_k_m.gguf")
}

fn create_orchestrator() -> Orchestrator {
    let mut model = llama_cpp_provider::LlamaCppModel::new();
    let config = ResolvedConfig {
        model_path: test_model_path(),
        ..ResolvedConfig::default()
    };
    model.load(&config).expect("failed to load model");

    let info = ProviderInfo {
        name: "llama.cpp".into(),
        version: "0.1.0".into(),
        supported_capabilities: vec![CapabilityKind::BuildSummary, CapabilityKind::ExplainRust],
    };
    let mut provider = LocalProvider::new(model, config, info);
    provider.load().expect("failed to init provider");

    Orchestrator::new(
        std::sync::Arc::new(provider),
        std::sync::Arc::new(NullContextSource),
        std::sync::Arc::new(DefaultPromptAssembler),
        std::sync::Arc::new(DefaultOutputParser),
        std::sync::Arc::new(DefaultCapabilityResponseParser),
    )
}

#[test]
fn orchestration_build_summary_e2e() {
    let orch = create_orchestrator();

    let request = OrchestrationRequest {
        capability: CapabilityRequest::BuildSummary(BuildSummaryRequest {
            metadata: RequestMetadata::new(),
            command: "cargo build".into(),
            exit_code: 1,
            stdout: String::new(),
            stderr: "error[E0599]: no method named `foo` found\n --> src/main.rs:5:10".into(),
            prompt: "Summarize the build failure".into(),
            temperature: 0.1,
            max_tokens: 256,
        }),
        context: None,
    };

    let resp = orch.execute(request).unwrap();
    assert!(!resp.text().is_empty(), "response text should not be empty");
    assert!(
        resp.text().len() > 10,
        "response should have substance: {}",
        resp.text()
    );
}

#[test]
fn orchestration_explain_rust_e2e() {
    let orch = create_orchestrator();

    let source = "fn main() {\n    let x = String::from(\"hello\");\n    let y = x;\n    println!(\"{}\", y);\n}";
    let request = OrchestrationRequest {
        capability: CapabilityRequest::ExplainRust(ExplainRustRequest {
            metadata: RequestMetadata::new(),
            source: source.into(),
            question: "What happens to x?".into(),
            prompt: format!("Explain this Rust code:\n{}", source),
            temperature: 0.1,
            max_tokens: 256,
        }),
        context: None,
    };

    let resp = orch.execute(request).unwrap();
    assert!(!resp.text().is_empty(), "response text should not be empty");
    assert!(
        resp.text().len() > 10,
        "response should have substance: {}",
        resp.text()
    );
}
